...
! 2006: Converted to free form source (F90) by Elizabeth Hunke
      ...
      real (kind=dbl_kind), dimension(:,:,:), allocatable, &
         ...
         incraice , & ! increment for aice into blocks          (m)
         incrhice , & ! increment for hice into blocks          (m)
      ...
      real (kind=dbl_kind), dimension(:,:), allocatable,public :: &
         incraice_glob , & !globally aggregated s-i-concentration increment
         incrhice_glob , & !globally aggregated s-i-thickness increment
         aice_glob     , & !globally aggregated ice concentration
         vice_glob         !globally aggregated s-i-volume
...
!
      ...
      allocate ( &
         ...
         incraice (nx_block,ny_block,max_blocks), & ! increment for aice into blocks          (m)
         incrhice (nx_block,ny_block,max_blocks), & ! increment for hice into blocks          (m)
         ...
         hicen_asm (nx_block,ny_block,ncat,max_blocks), &         ! thickness of ice
         incrhicen (nx_block,ny_block,ncat,max_blocks), &         ! increment in thickness (categorised)
         incraicen (nx_block,ny_block,ncat,max_blocks), &            ! increment in concentration
         ...
         incraice_glob (nx_global,ny_global), & !globally aggregated s-i-concentration increment
         incrhice_glob (nx_global,ny_global), & !globally aggregated s-i-thickness increment
         aice_glob     (nx_global,ny_global), & !globally aggregated ice concentration
         vice_glob     (nx_global,ny_global),&    !globally aggregated s-i-volume
