...
! 2008 ECH: moved ESMF code to its own driver
      ...
      use ice_restart_driver, only: ice_da_state_update
      use ice_restart_shared, only: ln_asmaice, ln_asmboth, ln_asmiau, &
          nday_asmiau, restart
...
!         William H. Lipscomb, LANL
      ...
      if (restart .and. istep .eq. 0) then
         if (ln_asmiau) then
            !-----------------------------------------------------------------
            !                  Incremental Analysis Update
            !-----------------------------------------------------------------
            if (my_task == master_task) then
              write(nu_diag,*) '____________________________________'
              write(nu_diag,*) '|     SEA-ICE Assimilation is ON   |'
              write(nu_diag,*) '|__________________________________|'
              write(nu_diag,*) ''
              write(nu_diag,*)'ln_asmiau = .TRUE.'
            endif
            if (ln_asmboth .and. .not. ln_asmaice) then !Both SIC and SIT assimilation
                   if (my_task == master_task) then
                       write(nu_diag,*)'ln_asmboth = .TRUE.'
                       write(nu_diag,*) ''
                   endif
                   call ice_da_state_update(istep)              !Calling update routine for SIC and SIT
             else if (ln_asmaice .and. .not. ln_asmboth) then
                   if (my_task == master_task) then
                       write(nu_diag,*)'ln_asmaice = .TRUE.'
                       write(nu_diag,*) ''
                   endif
                   call ice_da_state_update(istep)              !Calling update routine for SIC
             else if (ln_asmboth .and. ln_asmaice) then
                   call abort_ice( &
                 'Sea Ice Assim: ln_asmaice and ln_asmboth cant be true')
             else if (.not. ln_asmboth .and. .not. ln_asmaice) then
                call abort_ice( &
               'Sea Ice Assim: ln_asmaice or ln_asmboth must be true')
             endif

         else if (.not. ln_asmiau) then
            !-----------------------------------------------------------------
            !                  Direct Ingestion
            !-----------------------------------------------------------------
            if (my_task == master_task) then
               ...
               write(nu_diag,*) '|__________________________________|'
               write(nu_diag,*)''
               write(nu_diag,*)'Direct Ingestion: '
               write(nu_diag,*)'ln_asmiau = ',ln_asmiau
               write(nu_diag,*)'ln_asmboth= ',ln_asmboth
               write(nu_diag,*)'ln_asmaice= ',ln_asmaice
            endif
            if (ln_asmboth .and. .not. ln_asmaice) then      !Both SIC and SIT assimilation
                if (my_task == master_task) then
                   write(nu_diag,*)'ln_asmboth = .TRUE.'
               ...
               'Sea Ice Assim: ln_asmaice and ln_asmboth cant be true')
                 ...
                 if (my_task == master_task) then
                   write(nu_diag,*) 'Without Sea Ice Asimilaiton'
                 endif
!                call abort_ice( &
!               'Sea Ice Assim: ln_asmaice or ln_asmboth must be true')
            endif
...
!         William H. Lipscomb, LANL
      ...
      use ice_restart_column, only: write_restart_age, write_restart_FY, &
          ...
          write_restart_snow
      use ice_restart_driver, only: dumpfile,ice_da_state_update
...
!MHRI: CHECK THIS OMP
      ...
      !-----------------------------------------------------------------
         ...
         !-------------------------------------------------------------------------
         ! Ice state variable update from Sea Ice Data Assimialtion
         ! (Optional) Update fields using increments from SIC and/or SIT
         ! for IAU ONLY
         !-------------------------------------------------------------------------

            if (ln_asmiau) then
              if (istep .gt. 0) then
                   if (ln_asmboth .and. .not. ln_asmaice) then        !SIC and SIT assim
                      call ice_da_state_update(istep)
                   else if (ln_asmaice .and. .not. ln_asmboth) then   !SIC assim
                      call ice_da_state_update(istep)
                   else if (ln_asmboth .and. ln_asmaice) then
                       call abort_ice( &
                    'Sea Ice Assim: ln_asmaice and ln_asmboth cant be true')
