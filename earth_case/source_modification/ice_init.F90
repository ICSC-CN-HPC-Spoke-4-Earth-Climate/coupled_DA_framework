...
!           Converted to free source form (F90).
      ...
      character(len=char_len_long),public :: &
         ...
         incr_file            ! 'default'  => latitude and sst dependent
                                  !    filename   => read file
                                !filename and path for the increment
                                !filename and path for seaice observation
...
! author Elizabeth C. Hunke, LANL
      ...
      use ice_restart_shared, only: &
          ...
          runid, runtype, use_restart_time, restart_format, lcdf64, ln_asmaice, &
          ln_asmboth, ln_asmiau, nday_asmiau
      ...
      namelist /setup_nml/ &
        ...
        ln_asmaice,     ln_asmboth,      ln_asmiau,      nday_asmiau,   &
...
!         file=__FILE__, line=__LINE__)
      ...
      incr_file    = ''  !Seaice increment file
      ...
      call broadcast_scalar(ln_asmaice,           master_task)
      call broadcast_scalar(ln_asmboth,           master_task)
      call broadcast_scalar(ln_asmiau,            master_task)
      call broadcast_scalar(nday_asmiau,          master_task)
      call broadcast_scalar(pointer_file,         master_task)
      call broadcast_scalar(ice_ic,               master_task)
      call broadcast_scalar(incr_file,            master_task)
...
!tcraig
      ...
      if (my_task == master_task) then
         ...
         if (restart .and. ln_asmaice) then
             write(nu_diag,1010) ' ln_asmaice                   = ', ln_asmaice
             write(nu_diag,*)    ' incr_file                =', trim(incr_file)
             if (ln_asmiau) then
                write(nu_diag,1010) ' ln_asmiau                   = ', ln_asmiau
                write(nu_diag,1010) ' nday_asmiau                 = ', nday_asmiau
             endif
         else if (restart .and. ln_asmboth) then
             write(nu_diag,1010) ' ln_asmboth                   = ', ln_asmboth
             write(nu_diag,*)    ' incr_file                =', trim(incr_file)
             if (ln_asmiau) then
                write(nu_diag,1010) ' ln_asmiau                   = ', ln_asmiau
                write(nu_diag,1010) ' nday_asmiau                 = ', nday_asmiau
