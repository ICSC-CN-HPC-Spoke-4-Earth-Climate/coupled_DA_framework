...
INTEGER(I4) :: IAUX,IST,IEN,NMAXOBS,JO,NOINT,KOBS,JSAT,K,KSST
...
DO JP=1,NPROCS
   ...
   IF(KKCNT(JP,NNINS) .GT. 0 ) THEN
      ...
      WRITE(*,*) 'INS MINMAX : ',JP,MINVAL(INSB%RES(IST:IEN)),&
      & MAXVAL(INSB%RES(IST:IEN))
...
!            IF( .NOT.ABS(INSB%RES(JOBS)).LT.1.E+9_R8 ) THEN
!                INSB%BAC(JOBS)=-999._R8
!                INSB%RES(JOBS)=-999._R8
!            ENDIF
        ENDIF 
   ...
   IF(KKCNT(JP,NNSLA) .GT. 0 ) THEN
      ...
      BAC=SLAB%BAC(IST:IEN),&
      TB=SLAB%TB(IST:IEN,1:KM),&
      SB=SLAB%SB(IST:IEN,1:KM),&
      BCP=SLAB%BCP(IST:IEN,1:NPRD_SLA),&
      RES=SLAB%RES(IST:IEN) )
      ...
      CYCSLA : DO JOBS=1,KKCNT(JP,NNSLA)
        ...
        SLA%BAC( KOBS )=SLAB%BAC(JOBS)
        SLA%RES( KOBS )=SLAB%RES(JOBS)
        SLA%TB( KOBS,1:KM )=SLAB%TB(JOBS,1:KM)
        SLA%SB( KOBS,1:KM )=SLAB%SB(JOBS,1:KM)
        SLA%BCP( KOBS,: )=SLAB%BCP(JOBS,:)
      ENDDO CYCSLA
   ...
   IF(KKCNT(JP,NNSST) .GT. 0 ) THEN
      ...
      JB=SSTB%JB(IST:IEN,:),&
      PQ=SSTB%PQ(IST:IEN,:),&
      BCP=SSTB%BCP(IST:IEN,1:NPRD_SST),&
      ERR=SSTB%ERR(IST:IEN),&
      BAC=SSTB%BAC(IST:IEN),&
      RES=SSTB%RES(IST:IEN) )
      ...
      CYCSST : DO JOBS=1,KKCNT(JP,NNSST)
        ...
        SST%ERR( KOBS )=SSTB%ERR(JOBS)
        SST%BAC( KOBS )=SSTB%BAC(JOBS)
        SST%RES( KOBS )=SSTB%RES(JOBS)
        SST%BCP( KOBS,: )=SSTB%BCP(JOBS,:)
      ENDDO CYCSST
   ...
   IF(KKCNT(JP,NNSSS) .GT. 0 ) THEN
      ...
      NLEVS=KM, NPREDS=NPRD_SSS,NPQ=NPQ,NPQ2=NPQ,&
      FLC=SSSB%FLC(IST:IEN),&
      NIND=SSSB%NIND(IST:IEN),&
      ...
      JB=SSSB%JB(IST:IEN,:),&
      PQ=SSSB%PQ(IST:IEN,:),&
      BAC=SSSB%BAC(IST:IEN),&
      BCP=SSSB%BCP(IST:IEN,:),&
      RES=SSSB%RES(IST:IEN) )
      ...
      CYCSSS : DO JOBS=1,KKCNT(JP,NNSSS)
        ...
        IF( KOBS .LT. 1 .OR. KOBS .GT. SSS%NO .OR. SSS%FLG( KOBS ) .EQ. 1 ) THEN
            ...
            WRITE(335,*) 'ERROR : PROC, JOBS, KOBS, SSS%NO, FLG',&
            & JP, JOBS,KOBS,SSS%NO,SSS%FLG( KOBS ),CSTR
        ...
        SSS%BCP( KOBS,: )=SSSB%BCP(JOBS,:)
        SSS%BAC( KOBS )=SSSB%BAC(JOBS)
        SSS%RES( KOBS )=SSSB%RES(JOBS)
      ENDDO CYCSSS
...
IF ( INS%NO .GT. 0 ) THEN
  KOBS=INS%NO
  OPEN(31,FILE='INSMIS_ALL.bin',FORM='UNFORMATTED',ACCESS='SEQUENTIAL')
...
IF ( SLA%NO .GT. 0 ) THEN
  KOBS=SLA%NO
  OPEN(32,FILE='SLAMIS_ALL.bin',FORM='UNFORMATTED',ACCESS='SEQUENTIAL')
...
IF ( SST%NO .GT. 0 ) THEN
  KOBS=SST%NO
  OPEN(33,FILE='SSTMIS_ALL.bin',FORM='UNFORMATTED',ACCESS='SEQUENTIAL')
...
IF ( SSS%NO .GT. 0 ) THEN
  KOBS=SSS%NO
  OPEN(34,FILE='SSSMIS_ALL.bin',FORM='UNFORMATTED',ACCESS='SEQUENTIAL')
