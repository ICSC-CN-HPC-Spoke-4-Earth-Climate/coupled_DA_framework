...
INTEGER(I4)      , ALLOCATABLE, DIMENSION(:)   :: USARGO_QC
...
WRITE(IOUN,*)
    ...
    IF(LLUSQC) THEN
          ALLOCATE(USARGO_QC(NPROFS))
          STAT = NF90_INQ_VARID (NCID, 'USARGO_QC', IDVAR)
          IF (STAT /= NF90_NOERR) CALL HANDLE_ERR(STAT)
          STAT = NF90_GET_VAR (NCID,IDVAR,USARGO_QC(1:NPROFS))
          IF (STAT /= NF90_NOERR) CALL HANDLE_ERR(STAT)
          WRITE(IOUN,*) 'USARGO_QC READ'
    ENDIF
...
!    UKMO SIMPLIFIED VERSION OF WMO 1770 TABLE
  ...
  IF (LLUSQC) THEN
   WRITE(IOUN,*) ' ARGO NOT COUNTED FROM US       : ',SUM(USARGO_QC)
  ENDIF
  ...
  IF (LLUSQC) DEALLOCATE(USARGO_QC)
