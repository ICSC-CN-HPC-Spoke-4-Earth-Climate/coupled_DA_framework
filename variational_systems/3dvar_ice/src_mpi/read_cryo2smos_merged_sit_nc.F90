SUBROUTINE READ_CRYO2SMOS_MERGED_SIT_NC(CFILE,MAXOBS,NTOBS,ZDATA,&
          & ZLLON,ZLLAT,ZERR)
...
WRITE(IOUNLOG,*) ' :: OPENING CRYOSAT2-SMOS MEGED SIT FILE '//TRIM(CFILE)
...
STAT = NF90_INQ_VARID (NCID, 'analysis_sea_ice_thickness', IDVAR)
IF (STAT /= NF90_NOERR) CALL HANDLE_ERR(STAT)
...
STAT = NF90_INQ_VARID (NCID, 'analysis_sea_ice_thickness_unc', IDVAR)
IF (STAT /= NF90_NOERR) CALL HANDLE_ERR(STAT)
...
DO JT=1,NT
 DO JY=1,NY
  DO JX=1,NX
        ...
        IF(ZOSIT(JX,JY,JT).GE. 0._R8 .AND. ZOSIT(JX,JY,JT).LE. 9._R8 .AND. &
        & ZOSIT(JX,JY,JT) /= MISSVALSIT .AND. ZOERR(JX,JY,JT) /=MISSVALERR ) THEN
          NTOBS=NTOBS+1
          IF(NTOBS.GT.MAXOBS) &
          & CALL ABOR1('READ_CRYO2-SMOS-MERGED_SIT_NC   : MAXOBS TO INCREASE')
          ZDATA(NTOBS) = REAL(ZOSIT(JX,JY,JT),KIND=R8)
          ZLLON(NTOBS) = REAL(ZOLON(JX,JY),KIND=R8)
          ZLLAT(NTOBS) = REAL(ZOLAT(JX,JY),KIND=R8)
          ZERR(NTOBS)  = REAL(ZOERR(JX,JY,JT),KIND=R8)*COEFF_SIT_ERR_CRYOSMOS
        ENDIF
  ENDDO
 ENDDO
ENDDO
...
END SUBROUTINE READ_CRYO2SMOS_MERGED_SIT_NC
