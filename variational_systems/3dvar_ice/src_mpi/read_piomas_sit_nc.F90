SUBROUTINE READ_PIOMAS_SIT_NC(CFILE,MAXOBS,NTOBS,ZDATA,&
          & ZLLON,ZLLAT,ZERR,MM)
...
WRITE(IOUNLOG,*) ' :: OPENING PIOMAS SIT FILE '//TRIM(CFILE)
...
STAT = NF90_INQ_VARID (NCID, 'ice_thickness', IDVAR)
IF (STAT /= NF90_NOERR) CALL HANDLE_ERR(STAT)
...
STAT=NF90_OPEN("piomas_std.nc",NF90_NOWRITE, NCID)
IF (STAT /= NF90_NOERR) CALL HANDLE_ERR(STAT)
 ...
 DO JY=1,NY
  DO JX=1,NX
        IF(ZOSIT(JX,JY,JT).GE. 0. .AND. ZOSIT(JX,JY,JT).LE. 10. ) THEN
          ...
          ZERR(NTOBS)  = REAL(ZOERR(JX,JY,1),KIND=R8)*COEFF_SIT_ERR_PIOMAS
         ENDIF
  ENDDO
 ENDDO
ENDDO
...
END SUBROUTINE READ_PIOMAS_SIT_NC
