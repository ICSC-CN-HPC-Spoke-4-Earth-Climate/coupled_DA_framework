...
USE SET_KND
USE OBSHANDLING, ONLY : LTHIN_SLA,LTHIN_INS,LTHIN_SST,LTHIN_SIC,LTHIN_SIT,&
                      & CINSTHINOPT,LDOUBLETHIN,&
                      & RTHIND_SLA_X,RTHIND_SLA_Y,&
                      & RTHIND_SST_X,RTHIND_SST_Y,&
                      & RTHIND_SIC_X,RTHIND_SIC_Y,&
                      & RTHIND_SIT_X,RTHIND_SIT_Y,&
...
IF( SIC%NO .GT. 0 ) THEN
IF( LTHIN_SIC ) CALL THINOUT_SIC(1,RTHIND_SIC_X,RTHIND_SIC_Y)

IF( LTHIN_SIC .AND.LDOUBLETHIN) CALL THINOUT_SIC(2,RTHIND_SIC_X,RTHIND_SIC_Y)
ENDIF

IF( SIT%NO .GT. 0 ) THEN
IF( LTHIN_SIT ) CALL THINOUT_SIT(1,RTHIND_SIT_X,RTHIND_SIT_Y)

IF( LTHIN_SIT .AND.LDOUBLETHIN) CALL THINOUT_SIT(2,RTHIND_SIT_X,RTHIND_SIT_Y)
ENDIF
