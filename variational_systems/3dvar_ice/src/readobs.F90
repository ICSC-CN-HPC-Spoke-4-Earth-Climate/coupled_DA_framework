...
USE SET_KND
USE OBSDEF
USE OBS_STR, ONLY : SST,INS,SLA,OBS,SSS,SIC,SIT
USE GRD_STR
USE IOUNITS
...
INTEGER(I4) :: NOBS , IFILE, KOSUM, JO, KR(6)
...
! LOAD SIC OBSERVATIONS

IF ( OBS%SIC .GT. 0 ) THEN
  WRITE(IOUNOUT,*) ' READING IN SIC OBSERVATIONS'
     CALL READSIC
ELSE
    SIC%NO=0
ENDIF

! LOAD SIT OBSERVATIONS

IF ( OBS%SIT .GT. 0 ) THEN
  WRITE(IOUNOUT,*) ' READING IN SIT OBSERVATIONS'
     CALL READSIT
ELSE
    SIT%NO=0
ENDIF
...
IF ( SIC%NO .GT. 0 ) THEN
  DO JO=1,SIC%NO
    IF( SIC%LON(JO) .LT. MIMA_LL(1) .OR. SIC%LON(JO) .GT. MIMA_LL(2) .OR. &
       SIC%LAT(JO) .LT. MIMA_LL(3) .OR. SIC%LAT(JO) .GT. MIMA_LL(4) ) THEN
        SIC%FLC(JO) = 0
        SIC%EVE(JO) = KEVE_PDOM
        KR(5)=KR(5)+1
    ENDIF
  ENDDO
ENDIF
...
IF ( SIT%NO .GT. 0 ) THEN
  DO JO=1,SIT%NO
    IF( SIT%LON(JO) .LT. MIMA_LL(1) .OR. SIT%LON(JO) .GT. MIMA_LL(2) .OR. &
       SIT%LAT(JO) .LT. MIMA_LL(3) .OR. SIT%LAT(JO) .GT. MIMA_LL(4) ) THEN
        SIT%FLC(JO) = 0
        SIT%EVE(JO) = KEVE_PDOM
        KR(6)=KR(6)+1
    ENDIF
  ENDDO
ENDIF
...
WRITE(IOUNLOG,*) ' SIC : ', KR(5)
WRITE(IOUNLOG,*) ' SIT : ', KR(6)
WRITE(IOUNLOG,*) 
