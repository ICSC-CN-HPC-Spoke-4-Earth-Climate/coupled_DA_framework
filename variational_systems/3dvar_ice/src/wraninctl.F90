SUBROUTINE WRANINCTL(IM,JM,KM,KT,LON,LAT,DEP,SAL,TEM)
USE SET_KND
USE NETCDF
USE MPIREL
IMPLICIT NONE
...
WRITE(CKT,'(I6.6)') KT
  ...
  IF(LMPION) THEN
     CALL CHECK( NF90_CREATE('ANINCR_'//CKT//'.NC'//'.'//TRIM(CMPIDOM), NF90_CLOBBER, NCID) )
  ELSE
     CALL CHECK( NF90_CREATE('ANINCR_'//CKT//'.NC', NF90_CLOBBER, NCID) )
  ENDIF
