...
CALL MYFRTPROF_WALL('DEFEXTGRD_LV: DEFINE EXTENDED GRID',0)
...
& REAL(COUNT(LLMSR(:,:,:)),KIND=R8)*100._R8/(NX*NY*NTOTV)
...
DO JLEV=1,NTOTV
   ...
   ISTPNS(:,:,JLEV) = NINT(CRX(:,:,JLEV)*RF_EFC / NEWDX(:,:) )+1
   JSTPNS(:,:,JLEV) = NINT(CRY(:,:,JLEV)*RF_EFC / NEWDY(:,:) )+1
   WHERE(ISTPNS(:,:,JLEV) .LT. NINT(RF_EFC)) ISTPNS(:,:,JLEV)=NINT(RF_EFC)
   WHERE(JSTPNS(:,:,JLEV) .LT. NINT(RF_EFC)) JSTPNS(:,:,JLEV)=NINT(RF_EFC)

   WRITE(IOUNLOG,*) 'INITIAL ISTPNS: ',JLEV,RF_EFC,MINVAL(GRD%DX(:,:)),&
   & MAXVAL(GRD%DX(:,:)),SUM(GRD%DX(:,:))/(NX*NY),MINVAL(ISTPNS(:,:,JLEV)),&
   & MAXVAL(ISTPNS(:,:,JLEV)),SUM(ISTPNS(:,:,JLEV))/(NX*NY)
   WRITE(IOUNLOG,*) 'INITIAL JSTPNS: ',JLEV,RF_EFC,MINVAL(GRD%DY(:,:)),&
...
CALL MYFRTPROF_WALL('DEFEXTGRD_LV: DEFINE EXTENDED GRID',1)

END SUBROUTINE DEFEXTGRD_LV
