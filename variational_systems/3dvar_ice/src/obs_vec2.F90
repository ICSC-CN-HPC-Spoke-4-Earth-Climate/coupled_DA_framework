...
!-----------------------------------------------------------------------
 ...
 IMPLICIT NONE
  ...
  INTEGER(I4)    ::  NINS,NSLA,NSST,NSSS,NSIC,NSIT
...
! DEFINE OBSERVATIONAL VECTOR
  ...
  ENDIF
   ...
   IF( NSIC .GT. 0 ) THEN
     ZINNMIN(IGR) = MINVAL( SIC%RES( 1:SIC%NO )*REAL( SIC%FLC(1:SIC%NO), KIND=R8) )
     ZINNMAX(IGR) = MAXVAL( SIC%RES( 1:SIC%NO )*REAL( SIC%FLC(1:SIC%NO), KIND=R8) )
   ENDIF
   IGR=IGR+1
   IF( NSIT .GT. 0 ) THEN
     ZINNMIN(IGR) = MINVAL( SIT%RES( 1:SIT%NO )*REAL( SIT%FLC(1:SIT%NO),KIND=R8) )
     ZINNMAX(IGR) = MAXVAL( SIT%RES( 1:SIT%NO )*REAL( SIT%FLC(1:SIT%NO),KIND=R8) )
   ENDIF
   ...
   WRITE(IOUNLOG,*) '  SIC: ',ZINNMIN(5),ZINNMAX(5)
   WRITE(IOUNLOG,*) '  SIT: ',ZINNMIN(6),ZINNMAX(6)
   WRITE(IOUNLOG,*)
...
IF(LLPERTOBS) THEN
    ...
    DO I=1,SIC%NO
     IF(SIC%FLC(I).EQ.1)THEN
      K=K+1
      SIC%RES(I)=SIC%RES(I)+ZXR(K)*SIC%ERR(K)*ZRFACT
      SIC%VAL(I)=SIC%VAL(I)+ZXR(K)*SIC%ERR(K)*ZRFACT
     ENDIF
    ENDDO
    ...
    DO I=1,SIT%NO
     IF(SIT%FLC(I).EQ.1)THEN
      K=K+1
      SIT%RES(I)=SIT%RES(I)+ZXR(K)*SIT%ERR(K)*ZRFACT
      SIT%VAL(I)=SIT%VAL(I)+ZXR(K)*SIT%ERR(K)*ZRFACT
     ENDIF
    ENDDO
