...
NGOOD=SUM(SIC%FLC(1:SIC%NO))
NEWSIC%NO=NGOOD
NEWSIC%NC=NGOOD
ALLOCATE(NEWSIC%LON(NEWSIC%NO),&
        &NEWSIC%LAT(NEWSIC%NO),&
        &NEWSIC%VAL(NEWSIC%NO),&
        &NEWSIC%NIND(NEWSIC%NO),&
        &NEWSIC%TDIST(NEWSIC%NO),&
        &NEWSIC%FLG(NEWSIC%NO),&
        &NEWSIC%FLC(NEWSIC%NO),&
        &NEWSIC%KSAT(NEWSIC%NO),&
        &NEWSIC%EVE(NEWSIC%NO),&
        &NEWSIC%INC(NEWSIC%NO),&
        &NEWSIC%BAC(NEWSIC%NO),&
        &NEWSIC%TRA_BAC(NEWSIC%NO),&
        &NEWSIC%BIA(NEWSIC%NO),&
        &NEWSIC%ERR(NEWSIC%NO),&
        &NEWSIC%BGERR(NEWSIC%NO),&
        &NEWSIC%RES(NEWSIC%NO),&
        &NEWSIC%B_A(NEWSIC%NO),&
        &NEWSIC%IB (NEWSIC%NO,NPQ),&
        &NEWSIC%JB (NEWSIC%NO,NPQ),&
        &NEWSIC%PQ(NEWSIC%NO,NPQ),&
        &NEWSIC%IBH(NEWSIC%NO,NPQ),&
        &NEWSIC%JBH(NEWSIC%NO,NPQ),&
        &NEWSIC%PQH(NEWSIC%NO,NPQ),&
        &NEWSIC%TRACK(NEWSIC%NO),&
        &NEWSIC%BCP(NEWSIC%NO,NPRD_SIC),&
        &NEWSIC%TIM(NEWSIC%NO))

K=0
SICLOOP : DO JOBS=1,SIC%NO
   IF(SIC%FLC(JOBS) .EQ.1 ) THEN
   K=K+1
        NEWSIC%LON(K)=SIC%LON(JOBS)
        NEWSIC%LAT(K)=SIC%LAT(JOBS)
        NEWSIC%VAL(K)=SIC%VAL(JOBS)
        NEWSIC%NIND(K)=SIC%NIND(JOBS)
        NEWSIC%TDIST(K)=SIC%TDIST(JOBS)
        NEWSIC%FLG(K)=SIC%FLG(JOBS)
        NEWSIC%FLC(K)=SIC%FLC(JOBS)
        NEWSIC%KSAT(K)=SIC%KSAT(JOBS)
        NEWSIC%EVE(K)=SIC%EVE(JOBS)
        NEWSIC%INC(K)=SIC%INC(JOBS)
        NEWSIC%BAC(K)=SIC%BAC(JOBS)
        NEWSIC%TRA_BAC(K)=SIC%TRA_BAC(JOBS)
        NEWSIC%BIA(K)=SIC%BIA(JOBS)
        NEWSIC%ERR(K)=SIC%ERR(JOBS)
        NEWSIC%BGERR(K)=SIC%BGERR(JOBS)
        NEWSIC%RES(K)=SIC%RES(JOBS)
        NEWSIC%B_A(K)=SIC%B_A(JOBS)
        NEWSIC%IB (K,:)=SIC%IB (JOBS,:)
        NEWSIC%JB (K,:)=SIC%JB (JOBS,:)
        NEWSIC%PQ(K,:)=SIC%PQ(JOBS,:)
        NEWSIC%TRACK(K)=SIC%TRACK(JOBS)
        NEWSIC%TIM(K)=SIC%TIM(JOBS)
        NEWSIC%IBH(K,:)=SIC%IBH(JOBS,:)
        NEWSIC%JBH(K,:)=SIC%JBH(JOBS,:)
        NEWSIC%PQH(K,:)=SIC%PQH(JOBS,:)
        NEWSIC%BCP(K,:)=SIC%BCP(JOBS,:)
   ENDIF
ENDDO SICLOOP
NEWSIC%FLC = 1
...
SIC%NO=NEWSIC%NO
SIC%NC=NEWSIC%NO
SICLOOP2 : DO JOBS=1,SIC%NO
       SIC%LON(JOBS)=NEWSIC%LON(JOBS)
       SIC%LAT(JOBS)=NEWSIC%LAT(JOBS)
       SIC%VAL(JOBS)=NEWSIC%VAL(JOBS)
       SIC%NIND(JOBS)=NEWSIC%NIND(JOBS)
       SIC%TDIST(JOBS)=NEWSIC%TDIST(JOBS)
       SIC%FLG(JOBS)=NEWSIC%FLG(JOBS)
       SIC%FLC(JOBS)=NEWSIC%FLC(JOBS)
       SIC%KSAT(JOBS)=NEWSIC%KSAT(JOBS)
       SIC%EVE(JOBS)=NEWSIC%EVE(JOBS)
       SIC%INC(JOBS)=NEWSIC%INC(JOBS)
       SIC%BAC(JOBS)=NEWSIC%BAC(JOBS)
       SIC%TRA_BAC(JOBS)=NEWSIC%TRA_BAC(JOBS)
       SIC%BIA(JOBS)=NEWSIC%BIA(JOBS)
       SIC%ERR(JOBS)=NEWSIC%ERR(JOBS)
       SIC%BGERR(JOBS)=NEWSIC%BGERR(JOBS)
       SIC%RES(JOBS)=NEWSIC%RES(JOBS)
       SIC%B_A(JOBS)=NEWSIC%B_A(JOBS)
       SIC%IB (JOBS,:)=NEWSIC%IB (JOBS,:)
       SIC%JB (JOBS,:)=NEWSIC%JB (JOBS,:)
       SIC%PQ(JOBS,:)=NEWSIC%PQ(JOBS,:)
       SIC%TRACK(JOBS)=NEWSIC%TRACK(JOBS)
       SIC%TIM(JOBS)=NEWSIC%TIM(JOBS)
       SIC%IBH(JOBS,:)=NEWSIC%IBH(JOBS,:)
       SIC%JBH(JOBS,:)=NEWSIC%JBH(JOBS,:)
       SIC%PQH(JOBS,:)=NEWSIC%PQH(JOBS,:)
       SIC%BCP(JOBS,:)=NEWSIC%BCP(JOBS,:)
ENDDO SICLOOP2

DEALLOCATE(NEWSIC%LON,&
        &NEWSIC%LAT,&
        &NEWSIC%VAL,&
        &NEWSIC%TDIST,&
        &NEWSIC%FLG,&
...
NGOOD=SUM(SIT%FLC(1:SIT%NO))
NEWSIT%NO=NGOOD
NEWSIT%NC=NGOOD
ALLOCATE(NEWSIT%LON(NEWSIT%NO),&
        &NEWSIT%LAT(NEWSIT%NO),&
        &NEWSIT%VAL(NEWSIT%NO),&
        &NEWSIT%NIND(NEWSIT%NO),&
        &NEWSIT%TDIST(NEWSIT%NO),&
        &NEWSIT%FLG(NEWSIT%NO),&
        &NEWSIT%FLC(NEWSIT%NO),&
        &NEWSIT%KSAT(NEWSIT%NO),&
        &NEWSIT%EVE(NEWSIT%NO),&
        &NEWSIT%INC(NEWSIT%NO),&
        &NEWSIT%BAC(NEWSIT%NO),&
        &NEWSIT%TRA_BAC(NEWSIT%NO),&
        &NEWSIT%BIA(NEWSIT%NO),&
        &NEWSIT%ERR(NEWSIT%NO),&
        &NEWSIT%BGERR(NEWSIT%NO),&
        &NEWSIT%RES(NEWSIT%NO),&
        &NEWSIT%B_A(NEWSIT%NO),&
        &NEWSIT%IB (NEWSIT%NO,NPQ),&
        &NEWSIT%JB (NEWSIT%NO,NPQ),&
        &NEWSIT%PQ(NEWSIT%NO,NPQ),&
        &NEWSIT%IBH(NEWSIT%NO,NPQ),&
        &NEWSIT%JBH(NEWSIT%NO,NPQ),&
        &NEWSIT%PQH(NEWSIT%NO,NPQ),&
        &NEWSIT%TRACK(NEWSIT%NO),&
        &NEWSIT%BCP(NEWSIT%NO,NPRD_SIT),&
        &NEWSIT%TIM(NEWSIT%NO))

K=0
SITLOOP : DO JOBS=1,SIT%NO
   IF(SIT%FLC(JOBS) .EQ.1 ) THEN
   K=K+1
        NEWSIT%LON(K)=SIT%LON(JOBS)
        NEWSIT%LAT(K)=SIT%LAT(JOBS)
        NEWSIT%VAL(K)=SIT%VAL(JOBS)
        NEWSIT%NIND(K)=SIT%NIND(JOBS)
        NEWSIT%TDIST(K)=SIT%TDIST(JOBS)
        NEWSIT%FLG(K)=SIT%FLG(JOBS)
        NEWSIT%FLC(K)=SIT%FLC(JOBS)
        NEWSIT%KSAT(K)=SIT%KSAT(JOBS)
        NEWSIT%EVE(K)=SIT%EVE(JOBS)
        NEWSIT%INC(K)=SIT%INC(JOBS)
        NEWSIT%BAC(K)=SIT%BAC(JOBS)
        NEWSIT%TRA_BAC(K)=SIT%TRA_BAC(JOBS)
        NEWSIT%BIA(K)=SIT%BIA(JOBS)
        NEWSIT%ERR(K)=SIT%ERR(JOBS)
        NEWSIT%BGERR(K)=SIT%BGERR(JOBS)
        NEWSIT%RES(K)=SIT%RES(JOBS)
        NEWSIT%B_A(K)=SIT%B_A(JOBS)
        NEWSIT%IB (K,:)=SIT%IB (JOBS,:)
        NEWSIT%JB (K,:)=SIT%JB (JOBS,:)
        NEWSIT%PQ(K,:)=SIT%PQ(JOBS,:)
        NEWSIT%TRACK(K)=SIT%TRACK(JOBS)
        NEWSIT%TIM(K)=SIT%TIM(JOBS)
        NEWSIT%IBH(K,:)=SIT%IBH(JOBS,:)
        NEWSIT%JBH(K,:)=SIT%JBH(JOBS,:)
        NEWSIT%PQH(K,:)=SIT%PQH(JOBS,:)
        NEWSIT%BCP(K,:)=SIT%BCP(JOBS,:)
   ENDIF
ENDDO SITLOOP
NEWSIT%FLC = 1
...
SIT%NO=NEWSIT%NO
SIT%NC=NEWSIT%NO
SITLOOP2 : DO JOBS=1,SIT%NO
       SIT%LON(JOBS)=NEWSIT%LON(JOBS)
       SIT%LAT(JOBS)=NEWSIT%LAT(JOBS)
       SIT%VAL(JOBS)=NEWSIT%VAL(JOBS)
       SIT%NIND(JOBS)=NEWSIT%NIND(JOBS)
       SIT%TDIST(JOBS)=NEWSIT%TDIST(JOBS)
       SIT%FLG(JOBS)=NEWSIT%FLG(JOBS)
       SIT%FLC(JOBS)=NEWSIT%FLC(JOBS)
       SIT%KSAT(JOBS)=NEWSIT%KSAT(JOBS)
       SIT%EVE(JOBS)=NEWSIT%EVE(JOBS)
       SIT%INC(JOBS)=NEWSIT%INC(JOBS)
       SIT%BAC(JOBS)=NEWSIT%BAC(JOBS)
       SIT%TRA_BAC(JOBS)=NEWSIT%TRA_BAC(JOBS)
       SIT%BIA(JOBS)=NEWSIT%BIA(JOBS)
       SIT%ERR(JOBS)=NEWSIT%ERR(JOBS)
       SIT%BGERR(JOBS)=NEWSIT%BGERR(JOBS)
       SIT%RES(JOBS)=NEWSIT%RES(JOBS)
       SIT%B_A(JOBS)=NEWSIT%B_A(JOBS)
       SIT%IB (JOBS,:)=NEWSIT%IB (JOBS,:)
       SIT%JB (JOBS,:)=NEWSIT%JB (JOBS,:)
       SIT%PQ(JOBS,:)=NEWSIT%PQ(JOBS,:)
       SIT%TRACK(JOBS)=NEWSIT%TRACK(JOBS)
       SIT%TIM(JOBS)=NEWSIT%TIM(JOBS)
       SIT%IBH(JOBS,:)=NEWSIT%IBH(JOBS,:)
       SIT%JBH(JOBS,:)=NEWSIT%JBH(JOBS,:)
       SIT%PQH(JOBS,:)=NEWSIT%PQH(JOBS,:)
       SIT%BCP(JOBS,:)=NEWSIT%BCP(JOBS,:)
ENDDO SITLOOP2

DEALLOCATE(NEWSIT%LON,&
        &NEWSIT%LAT,&
        &NEWSIT%VAL,&
        &NEWSIT%TDIST,&
        &NEWSIT%FLG,&
