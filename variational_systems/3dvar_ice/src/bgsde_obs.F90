...
SUBROUTINE BGSDE_OBS
  ...
  INTEGER (I4)               :: KT2, KQ2,NKSIT,NKSIC
  ...
  REAL(R8)       :: SIC2 (GRD%IM,GRD%JM)
  REAL(R8)       :: SIT2 (GRD%IM,GRD%JM)
...
ENDIF
  ...
  IF(LL_ICE) THEN
     NKSIC=2*GRD%KM*COUNT( (/LL_TS/) ) +COUNT( (/LL_SSH/) )+ 2* COUNT( (/LL_TQ2/) ) +1
     NKSIT=NKSIC+1
     DO J=1,GRD%JM
      DO I=1,GRD%IM
        KI=GRD%REG(I,J)
        ! SIC
        SIC2(I,J) = SQRT( SUM( ROS%EVC(KI,NKSIC,:) * &
        & ROS%EVA(KI,:) * ROS%EVA(KI,:) * ROS%EVC(KI,NKSIC,:) ) )
        ! SIT
        SIT2(I,J) = SQRT( SUM( ROS%EVC(KI,NKSIT,:) * &
        & ROS%EVA(KI,:) * ROS%EVA(KI,:) * ROS%EVC(KI,NKSIT,:) ) )
      ENDDO
     ENDDO

CALL WRITE_VARNCDF('SICBG_STDEV_'//TRIM(CMPIDOM)//'.NC',&
      & GRD%IM,GRD%JM,SIC2,'bgsd',&
      & ' ','background-error st.dev.')
CALL WRITE_VARNCDF('SITBG_STDEV_'//TRIM(CMPIDOM)//'.NC',&
      & GRD%IM,GRD%JM,SIT2,'bgsd',&
      & ' ','background-error st.dev.')
...
LOOPSIC : DO JOBS=1,SIC%NO
     ...
     IF(SIC%FLC(JOBS) .NE. 1 ) CYCLE LOOPSIC

     SIC%BGERR(JOBS) = &
     & OSUM( SIC%PQ(JOBS,1:NPQ),SIC2(SIC%IB(JOBS,1:NPQ),SIC%JB(JOBS,1:NPQ) ) )
...
LOOPSIT : DO JOBS=1,SIT%NO
     ...
     IF(SIT%FLC(JOBS) .NE. 1 ) CYCLE LOOPSIT

     SIT%BGERR(JOBS) = &
     & OSUM( SIT%PQ(JOBS,1:NPQ),SIT2(SIT%IB(JOBS,1:NPQ),SIT%JB(JOBS,1:NPQ) ) )
