SUBROUTINE WRANINCHR(IM,JM,KM,EOF,LON,LAT,DEP,SAL,TEM,RO)
USE SET_KND
USE NETCDF
USE MPIREL
IMPLICIT NONE
INTEGER(I4), INTENT(IN) :: IM, JM, KM, EOF
REAL(R8), INTENT(IN) :: LON(IM,JM),LAT(IM,JM),DEP(KM)
REAL(R8), INTENT(IN) :: SAL(IM,JM,KM),TEM(IM,JM,KM)
REAL(R8), INTENT(IN) :: RO(IM,JM,EOF)
INTEGER(I4) :: NCID,DIMIDS1(1),DIMIDS2(2),DIMIDS3(3),DIMIDS4(3)
INTEGER(I4) :: VARID(6),X_DIMID,Y_DIMID,Z_DIMID,E_DIMID

  IF(LMPION) THEN
     CALL CHECK( NF90_CREATE('ANINCR_HR.NC'//'.'//TRIM(CMPIDOM), NF90_CLOBBER, NCID) )
  ELSE
     CALL CHECK( NF90_CREATE('ANINCR_HR.NC', NF90_CLOBBER, NCID) )
  ENDIF
  ...
  CALL CHECK( NF90_DEF_DIM(NCID, 'eof', EOF, E_DIMID) )
  DIMIDS3=  (/X_DIMID, Y_DIMID, Z_DIMID /)
  DIMIDS4=  (/X_DIMID, Y_DIMID, E_DIMID /)
  ...
  CALL CHECK( NF90_DEF_VAR(NCID, 'INCRO',NF90_REAL, DIMIDS4, VARID(6)) )
  ...
  CALL CHECK( NF90_PUT_ATT(NCID, VARID(6), 'longname','Control Variable Increment') )
  ...
  CALL CHECK( NF90_PUT_VAR(NCID, VARID(6), RO ) )
  CALL CHECK( NF90_CLOSE(NCID) )

END SUBROUTINE WRANINCHR
