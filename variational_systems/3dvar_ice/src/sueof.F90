...
! EOFS
     ...
     CALL FLUSH(IOUNLOG)
     IF (.NOT. LL_ICE .OR. (LL_ICE .AND. .NOT. LL_TS) ) THEN
             ALLOCATE ( GRD%RO( GRD%IM, GRD%JM, ROS%NEOF))
             ALLOCATE ( GRD%RO_AD( GRD%IM, GRD%JM, ROS%NEOF))
             ALLOCATE ( ROS%EVC( ROS%NREG, ROS%KMT, ROS%NEOF), ROS%EVA( ROS%NREG,ROS%NEOF) )
     ...
     ELSE
             ...
             ALLOCATE ( GRD%RO( GRD%IM, GRD%JM, ROS%NEOF+2))
             ALLOCATE ( GRD%RO_AD( GRD%IM, GRD%JM, ROS%NEOF+2))
             ALLOCATE ( ROS%EVC( ROS%NREG, ROS%KMT+2, ROS%NEOF+2), ROS%EVA(ROS%NREG,ROS%NEOF+2) )
             
             ROS%EVC=0._R8
             ROS%EVA=0._R8

             WRITE(IOUNLOG,*) ' EOF WITH ICE AND TS,READING TS FIRST..' 
             CALL READ_NCEOF(TRIM(ROS%EOF_FILE),ROS%EOGNX,ROS%EOGNY,ROS%NREG,GRD%IM,GRD%JM,ROS%KMT,ROS%NEOF,ROS%EVA(1:ROS%NREG,1:ROS%NEOF),ROS%EVC(1:ROS%NREG,1:ROS%KMT,1:ROS%NEOF),GRD%REG,LSQUAREDEVA)
             WRITE(IOUNLOG,*) ' EOF WITH ICE AND TS,CALLING ICE NOW.. REGIONS MUST BE THE SAME IN THIS FORMULATION'

             ALLOCATE(ROS%EVC_ICE_SEP( ROS%NREG,2,2), ROS%EVA_ICE_SEP(ROS%NREG,2) )
             CALL READ_NCEOF(TRIM(ROS%EOF_FILE_ICE_SEP),ROS%EOGNX,ROS%EOGNY,ROS%NREG,GRD%IM,GRD%JM,2,2,ROS%EVA_ICE_SEP,ROS%EVC_ICE_SEP,GRD%REG,.FALSE.)             
             
             ! The addition of new ice eofs comes with the additio of 2 more
             !eofs that are only dedicated to the sea-ice and any correlation
             !between TS and ice will be zero.

             ROS%EVA(1:ROS%NREG,(ROS%NEOF+1):(ROS%NEOF+2))=ROS%EVA_ICE_SEP
             ROS%EVC(1:ROS%NREG,(ROS%KMT+1):(ROS%KMT+2),(ROS%NEOF+1):(ROS%NEOF+2))=ROS%EVC_ICE_SEP

             !update with all eofs and levels
             ROS%NEOF=ROS%NEOF+2
             ROS%KMT=ROS%KMT+2
             
             DEALLOCATE(ROS%EVC_ICE_SEP)
             DEALLOCATE(ROS%EVA_ICE_SEP)        
     ...
     IF(REDNMC.GT.0.1_R8 .AND. REDNMC.LT.10._R8 ) THEN
         IF ( LL_ICE .AND. LL_TS) THEN
               WRITE(IOUNLOG,*)
               WRITE(IOUNLOG,*) ' *** USING REDNMC FOR TS == ',REDNMC
               WRITE(IOUNLOG,*) 
               ROS%EVA(1:ROS%NREG,1:(ROS%NEOF-2))=ROS%EVA(1:ROS%NREG,1:(ROS%NEOF-2))*REDNMC
!        ELSEIF ( .NOT. LL_ICE .AND. LL_TS) THE
         ELSE
                ROS%EVA=ROS%EVA*REDNMC
               WRITE(IOUNLOG,*)
               WRITE(IOUNLOG,*) ' *** USING REDNMC FOR ALL  == ',REDNMC
               WRITE(IOUNLOG,*)
        ENDIF
     ENDIF
     ...
     IF(LL_BGFACT) THEN 
       ...
       CNT = 0
       BGFAC1 = 0._R8
       WRITE(IOUNLOG,*) ' READING GRID-DEPENDENT INFLATION OF B (FOR TS)'
       ...
       IF (  LL_ICE .AND. LL_TS) THEN
              ...
              ENDDO
       ELSEIF ( .NOT. LL_ICE .AND. LL_TS) THEN
              DO KK=1,ROS%NEOF
                 ROS%EVA(:,KK)=ROS%EVA(:,KK)*BGFAC1
              ENDDO   
       ENDIF
     ...
     ! masks only the vertical 3D TS 

     DO KK=1,ROS%NEOF
      DO J=1,GRD%JM
       DO I=1,GRD%IM
          IF (.NOT. LL_ICE .AND. LL_TS) THEN
          ...
          ELSEIF (LL_ICE .AND. LL_TS) THEN
                  ROS%EVC(GRD%REG(I,J),1:GRD%KM,KK)=ROS%EVC(GRD%REG(I,J),1:GRD%KM,KK)*GRD%MSK(I,J,:)
                  ROS%EVC(GRD%REG(I,J),(GRD%KM+3):(2*(GRD%KM+2)),KK)=ROS%EVC(GRD%REG(I,J),(GRD%KM+3):(2*(GRD%KM+2)),KK)*GRD%MSK(I,J,:)        
          ENDIF
     ...
     IF( LL_MLD_VLOC ) THEN
        ...
        ELSEIF (LL_ICE .AND. LL_TS) THEN
           DO K=2,PSV3D
              GRD%MVLOC(:,:,((K-1)*(GRD%KM+2)+1):((K-1)*(GRD%KM+2)+GRD%KM))=GRD%MVLOC(:,:,1:GRD%KM)
           ENDDO
        ENDIF
     ENDIF
     
     IF (LL_TS .AND. LL_UNMASK_EOFS) THEN
      DO K=2,GRD%KM
       KP=0
       KP2=0
       KP3=0
        ...
        DO I=1,GRD%IM 
         ...
         IF( ALL(ROS%EVC(KK,K,:).EQ.0._R8) .AND. GRD%MSK(I,J,K).GT.0.9_R8 ) THEN
           ...
           ELSE
              ...
              IF( .NOT. LL_ICE .AND. LL_TS) ROS%EVC(KK,K+GRD%KM,:) = ROS%EVC(KK,K+GRD%KM-1,:)
              IF( LL_ICE .AND. LL_TS) ROS%EVC(KK,K+(GRD%KM+2),:) = ROS%EVC(KK,K+(GRD%KM+2)-1,:)
           ENDIF
         ENDIF
        ENDDO
       ENDDO
