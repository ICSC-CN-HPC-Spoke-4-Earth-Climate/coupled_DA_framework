!$Id: mathelp.f90 11 2007-03-12 16:01:04Z bellier $
!
MODULE mathelp
!---------------------------------------------------------------------
...
!- a F90 environnement
!-
!-sc   V1d = reshape(V3id (ox:ox-1+sx,oy:oy-1+sy,oz:oz-1+sz),
!-sc                 SHAPE= (/sx*sy*sz/) )
!-
!- INPUT
...
!---------------------------------------------------------------------
!-
! We have to consider the case where the zoom starts at (1,1,1)
! but does not go over the full size.
!-
  IF (     (ox .EQ. 1).AND.(oy.EQ. 1).AND.(oz.EQ. 1) &
 &    .AND.(sx .EQ. xsz).AND.(sy .EQ. ysz).AND.(sz .EQ. zsz) ) THEN 
    DO ic=1,MAX(sx,1)*MAX(sy,1)*MAX(sz,1) 
      v1d(ic) = v3d(ic,1,1)
    ENDDO 
  ELSE IF (     (ox .EQ. 1).AND.(oy .EQ. 1) &
 &         .AND.(sx .EQ. xsz).AND.(sy .EQ. ysz)) THEN 
    DO iz=oz,(oz-1+sz) 
      DO ic=1,MAX(sx,1)*MAX(sy,1) 
        v1d(ic) = v3d(ic,1,iz)
      ENDDO
    ENDDO
  ELSE 
