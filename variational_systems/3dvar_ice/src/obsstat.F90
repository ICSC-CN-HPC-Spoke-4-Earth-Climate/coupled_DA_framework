...
STDET(1) = 0._R8
DO JO=1,SIC%NO
   IF(SIC%FLC(JO).EQ.1 .AND. SIC%FLG(JO).EQ.1) THEN
      COUNT(1) = COUNT(1) + 1
      MEANT(1)  = MEANT(1) + SIC%RES(JO)
   ENDIF
ENDDO
IF(COUNT(1).GT.0) MEANT(1) = MEANT(1)/REAL(COUNT(1))
DO JO=1,SIC%NO
   IF(SIC%FLC(JO).EQ.1 .AND. SIC%FLG(JO).EQ.1) THEN
      STDET(1)  = STDET(1) + &
      & (SIC%RES(JO)-MEANT(1))*(SIC%RES(JO)-MEANT(1))
   ENDIF
ENDDO
IF(COUNT(1).GT.1) STDET(1) = SQRT(STDET(1)/REAL(COUNT(1)-1))
WRITE(IOUNLOG,*)
WRITE(IOUNLOG,*) ' ** SIC OBSERVATIONS:'
WRITE(IOUNLOG,*) ' SIC : AMOUNT, BIAS, STDEV'
WRITE(IOUNLOG,*) COUNT(1), MEANT(1), STDET(1)
...
STDET(1) = 0._R8
DO JO=1,SIT%NO
   IF(SIT%FLC(JO).EQ.1 .AND. SIT%FLG(JO).EQ.1) THEN
      COUNT(1) = COUNT(1) + 1
      MEANT(1)  = MEANT(1) + SIT%RES(JO)
   ENDIF
ENDDO
IF(COUNT(1).GT.0) MEANT(1) = MEANT(1)/REAL(COUNT(1))
DO JO=1,SIT%NO
   IF(SIT%FLC(JO).EQ.1 .AND. SIT%FLG(JO).EQ.1) THEN
      STDET(1)  = STDET(1) + &
      & (SIT%RES(JO)-MEANT(1))*(SIT%RES(JO)-MEANT(1))
   ENDIF
ENDDO
IF(COUNT(1).GT.1) STDET(1) = SQRT(STDET(1)/REAL(COUNT(1)-1))
WRITE(IOUNLOG,*)
WRITE(IOUNLOG,*) ' ** SIT OBSERVATIONS:'
WRITE(IOUNLOG,*) ' SIT : AMOUNT, BIAS, STDEV'
WRITE(IOUNLOG,*) COUNT(1), MEANT(1), STDET(1)
