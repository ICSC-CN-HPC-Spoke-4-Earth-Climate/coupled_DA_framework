...
! TRANSPOSE CALCULATION IN THE PRESENSE OF COASTAL BOUNDARIES
 IF(DRV%MASK(DRV%KTR) .GT. 1) THEN ! DOUBLE THE TIME FOR RECURSIVE FILTER BUT
                                   ! RESULTS IS SELF_ADJOINT AS SHOULD BE THE
                                   ! _AD VECTORS ARE USED AS TEMPORAL ARRAY NOT
                                   ! AS _AD
...
! ---
! SIC  AND SIT ANAMORPH OPERATOR
    IF(DRV%ICEOP(DRV%KTR)  .EQ. 1 .AND. LL_ICE)  CALL ICE_ANAM

!WRITE(IOUNLOG,*) ' MINVAL SIC=',MINVAL( GRD%SIC(:,:) )
!WRITE(IOUNLOG,*) ' MAXVAL SIC=',MAXVAL( GRD%SIC(:,:) )
!WRITE(IOUNLOG,*) ' MINVAL SIT=',MINVAL( GRD%SIT(:,:) )
!WRITE(IOUNLOG,*) ' MAXVAL SIT=',MAXVAL( GRD%SIT(:,:) )

! WRITE(IOUNLOG,*)'after ICE_ANAM'
!CALL FLUSH(IOUNLOG)
