...
PUBLIC :: WRITE_VARNCDF, GETNCVAR, GETNCDIM,READ_ANAMORF_FILE
...
SUBROUTINE READ_ANAMORF_FILE(CFILE,dim,q,JPI,JPJ,SEASON,VAR,CVAR,qua_def,qua_ref)
USE SET_KND
USE NETCDF
IMPLICIT NONE
CHARACTER(LEN=*), INTENT(IN) :: CFILE,CVAR
INTEGER(I4), INTENT(IN) ::  dim,q,JPI,JPJ,SEASON
REAL(R8), INTENT(INOUT) :: VAR(dim,q,JPI,JPJ,SEASON)
REAL(R8), INTENT(INOUT) :: qua_def(q),qua_ref(q)
INTEGER(I4) :: NCID
INTEGER(I4) :: VARID
INTEGER(I4) :: I1, J1
I1=1 ; J1=1
IF( JPI .EQ. GRD%IM ) I1=GRD%I0
IF( JPJ .EQ. GRD%JM ) J1=GRD%J0
!WRITE(IOUNLOG,*) I1,J1,q,SEASON
 CALL CHECK( NF90_OPEN(CFILE,NF90_NOWRITE,NCID) )
 CALL CHECK( NF90_INQ_VARID(NCID,CVAR,VARID) )
 CALL CHECK( NF90_GET_VAR(NCID,VARID,VAR,START=(/1,1,I1,J1,1/),COUNT=(/dim,q,JPI,JPJ,SEASON/)) )
 CALL CHECK( NF90_INQ_VARID(NCID,"Q_INPUT",VARID) )
 CALL CHECK( NF90_GET_VAR(NCID,VARID,qua_def) )
 CALL CHECK( NF90_INQ_VARID(NCID,"Q_TARGET",VARID) )
 CALL CHECK( NF90_GET_VAR(NCID,VARID,qua_ref) )
 CALL CHECK( NF90_CLOSE(NCID) )
 WHERE (isnan(VAR) ) VAR=0._R8
 WHERE (abs(VAR) .GE. 1000000. ) VAR=0._R8

END SUBROUTINE READ_ANAMORF_FILE
END MODULE MYNETCDF
